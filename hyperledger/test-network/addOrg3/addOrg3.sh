...
function generateOrg3() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config="org${ORG}-crypto.yaml" --output="../organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org${ORG}.example.com/peers/peer0.org${ORG}.example.com/
  fi
...
function addOrg3 () {
  ...
  ${CONTAINER_CLI} exec cli ./scripts/org3-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE $ORG
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Org${ORG} peers to network"
  ${CONTAINER_CLI} exec cli ./scripts/org3-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE $ORG $PEER_PORT
  if [ $? -ne 0 ]; then
...
while [[ $# -ge 1 ]] ; do
  ...
  -p )
    CONFIG_PATH="$2"
    export ORG=$(cat ${CONFIG_PATH} | jq '.NAME')
    export CA_SERVER_PORT=$(cat ${CONFIG_PATH} | jq '.CA_SERVER_PORT') 
    export CA_USERNAME=$(cat ${CONFIG_PATH} | jq '.CA_USERNAME' | tr -d '"') 
    export CA_PASSWORD=$(cat ${CONFIG_PATH} | jq '.CA_PASSWORD' | tr -d '"') 

    export PEER_PORT=$(cat ${CONFIG_PATH} | jq '.PEER_PORT') 
    export PEER_CHAINCODE_PORT=$(cat ${CONFIG_PATH} | jq '.PEER_CHAINCODE_PORT') 

    # use this as the docker compose couch file
    COMPOSE_FILE_COUCH_BASE=compose/compose-couch-org${ORG}.yaml
    COMPOSE_FILE_COUCH_ORG3=compose/${CONTAINER_CLI}/docker-compose-couch-org${ORG}.yaml
    # use this as the default docker-compose yaml definition
    COMPOSE_FILE_BASE=compose/compose-org${ORG}.yaml
    COMPOSE_FILE_ORG3=compose/${CONTAINER_CLI}/docker-compose-org${ORG}.yaml
    # certificate authorities compose file
    COMPOSE_FILE_CA_BASE=compose/compose-ca-org${ORG}.yaml
    COMPOSE_FILE_CA_ORG3=compose/${CONTAINER_CLI}/docker-compose-ca-org${ORG}.yaml
    shift
    ;;
  * )
