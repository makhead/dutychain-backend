...
class Chaincode extends Contract {
	...
	async DeleteAsset(ctx, id) {
		...
		// delete the index
		let indexName = 'type-index';
		let indexKey = ctx.stub.createCompositeKey(indexName, [assetJSON.type, assetJSON.id]);
		if (!indexKey) {
	...
	async TransferAsset(ctx, id, newVendorId) {
		...
		if(assetToTransfer.type !== 'asset'){
			let err = {};
			err.error = 'Document Type is not asset!';
			
			throw new Error(err);
		}
		assetToTransfer.data.vendorId = newVendorId; //change the owner
	...
	/*
	async TransferAssetByType(ctx, vendorId, newVendorId) {
	...
	*/
	...
	async QueryAssetsByOwner(ctx, vendorId) {
		...
		queryString.selector.type = 'asset';
		queryString.selector.data.vendorId = vendorId;
